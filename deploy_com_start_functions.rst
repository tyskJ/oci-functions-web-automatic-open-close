Compute Instance起動用Functionsデプロイ
=====================================================================
.. note::
...
3-2. コード修正
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
* ``~/compute-start`` 配下の各種ファイルの中身を `本フォルダ配下のファイルの中身 <./envs/python/compute-start>`_ に置き換えてください

4. 関数のデプロイ
---------------------------------------------------------------------
.. code-block:: bash

  APP_NAME="compute-start"
  FN_NAME="compute-start"
  cd ~/${FN_NAME}
