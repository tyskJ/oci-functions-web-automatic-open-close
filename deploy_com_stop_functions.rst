Compute Instance停止用Functionsデプロイ
=====================================================================
.. note::
...
3-2. コード修正
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
* ``~/compute-stop`` 配下の各種ファイルの中身を `本フォルダ配下のファイルの中身 <./envs/python/compute-stop>`_ に置き換えてください

4. 関数のデプロイ
---------------------------------------------------------------------
.. code-block:: bash

  APP_NAME="compute-stop"
  FN_NAME="compute-stop"
  cd ~/${FN_NAME}
