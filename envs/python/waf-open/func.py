"""
Regional WAF の Request Access Rules の Default Actions を Allow Action に変更する OCI Functions
"""
...
def handler(ctx, data: io.BytesIO = None):
    ...
    try:
        ...
        if before_waf_policy_details.request_access_control.default_action_name == "Pre-configured Allow Action":
            return success_response(
                ctx,
                {
                    "message": "Already default action is Allow."
                },
                202
            )
        else:
            ...
            logger.info(
                "WAF Policy Default Action Update to Allow. "
                f"workRequestId={work_request_id}"
            )
            return success_response(
                ctx,
                {
                    "message": "WAF Policy Default Action Update to Allow.",
        ...
        logger.error(
            "Update WAF Policy to Allow failed: "
            f"status={e.status}, code={e.code}, message={e.message}"
        )
        return error_response(ctx, e.message, e.status)
    except Exception as e:
        logger.error(f"Unexpected error during update waf policy to allow: {e}")
        return error_response(ctx, str(e), 500)
