...
```
.
├── deploy_com_start_functions.rst    Compute Instance 起動用Functionsデプロイ手順
...
├── deploy_waf_open_functions.rst     WAF Request Access Rules Default Action をAllowに変更するFunctionsデプロイ手順
└── envs
    ...
    ├── python
    │   ├── compute-start             Compute Instance 開始用コード
    │   │   ├── func.py
    │   │   ├── func.yaml
    │   │   └── requirements.txt
    ...
    │   │   ├── func.yaml
    │   │   └── requirements.txt
    │   └── waf-open                  Default Action Allow化コード
    │       ├── func.py
    │       ├── func.yaml
    │       └── requirements.txt
    ├── resource_scheduler.tf         Resource Scheduler定義ファイル
